                ...
                >
                    {cases.map((c, i) => (
                        <motion.div
                            ...
                            className="bg-white dark:bg-[#1c1c1e] rounded-[2rem] overflow-hidden border border-slate-100 dark:border-white/10 shadow-lg hover:shadow-glow-sm transition-all group"
                        >
                            {/* Header */}
                            <div className={`bg-gradient-to-r ${c.color} p-6 relative overflow-hidden`}>
                                <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-bl-[40px] mix-blend-overlay"></div>
                                <div className="flex items-center gap-2 text-white/90 text-xs font-bold mb-3 uppercase tracking-wider">
                                    <c.icon size={16} /> {c.platform}
                                </div>
                                <h3 className="text-xl font-bold text-white mb-1 leading-tight">{c.title}</h3>
                                <span className="inline-block mt-2 text-[10px] bg-white/20 text-white px-3 py-1 rounded-full font-medium tracking-wide backdrop-blur-sm border border-white/20">{c.category}</span>
                            </div>
                            ...
                            <div className="p-6 md:p-8">
                                <div className="flex items-center justify-between mb-6">
                                    <div className="text-center">
                                        <div className="text-[10px] text-slate-400 uppercase font-bold mb-1 tracking-widest">Было</div>
                                        <div className="flex items-center gap-1">
                                            <Star size={16} className="text-slate-300 dark:text-slate-600 fill-slate-300 dark:fill-slate-600" />
                                            <span className="text-2xl font-bold text-slate-400">{c.ratingBefore}</span>
                                        </div>
                                        <div className="text-xs text-slate-400 mt-1">{c.reviewsBefore} отзывов</div>
                                    </div>

                                    <div className="flex-1 mx-4 relative">
                                        <div className="h-px bg-slate-200 dark:bg-white/10 w-full absolute top-1/2 -translate-y-1/2"></div>
                                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#0071e3] text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-sm whitespace-nowrap">
                                            {c.period}
                                        </div>
                                    </div>

                                    <div className="text-center">
                                        <div className="text-[10px] text-[#0071e3] uppercase font-bold mb-1 tracking-widest">Стало</div>
                                        <div className="flex items-center gap-1">
                                            <Star size={16} className="text-amber-400 fill-amber-400" />
                                            <span className="text-2xl font-black text-[#1d1d1f] dark:text-white">{c.ratingAfter}</span>
                                        </div>
                                        <div className="text-xs text-slate-500 mt-1 font-medium">{c.reviewsAfter} отзывов</div>
                                    </div>
                                </div>

                                {/* Result */}
                                <div className="bg-slate-50 dark:bg-white/5 rounded-2xl p-4 text-center border border-slate-100 dark:border-white/5 group-hover:bg-[#0071e3]/5 group-hover:border-[#0071e3]/20 transition-colors">
                                    <div className="text-sm font-bold text-[#1d1d1f] dark:text-white flex items-center justify-center gap-2">
                                        <TrendingUp size={18} className="text-[#0071e3]" /> {c.result}
                                    </div>
                                </div>
                            </div>
                        </motion.div>
